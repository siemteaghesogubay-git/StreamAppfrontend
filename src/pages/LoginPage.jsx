...
import { Mail, Lock, Eye, EyeOff, ArrowRight, Shield, Monitor, Users, Play } from 'lucide-react';
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.left}>
        ...
        <div style={s.tagline}>
          <h1 style={s.tagH1}>Din värld.<br />Vår kultur.<br /><span style={{ color: '#1db954' }}>Din stream.</span></h1>
      ...
      <div style={s.right}>
        <div style={s.formBox}>
          <div style={s.formHeader}>
            <h2 style={s.formTitle}>Logga in</h2>
            <p style={s.formSub}>Välkommen tillbaka till <span style={{ color: '#1db954' }}>STS Stream</span></p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={s.row}>
              ...
              <span style={s.forgot}>Glömt lösenord?</span>
            </div>

            <button style={s.submitBtn} type="submit" disabled={loading}>
              {loading ? 'Loggar in...' : <><span>Logga in</span> <ArrowRight size={18} /></>}
            </button>
          </form>

          <p style={s.registerText}>
            Inget konto? <Link to="/register" style={{ color: '#1db954', textDecoration: 'none', fontWeight: 500 }}>Registrera dig</Link>
          </p>
        </div>
      </div>
    </div>
...
const s = {
  ...
  row: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '16px 0' },
  checkLabel: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'rgba(255,255,255,0.6)', cursor: 'pointer' },
  forgot: { fontSize: 13, color: '#1db954', cursor: 'pointer' },
  submitBtn: { width: '100%', background: '#1db954', border: 'none', color: '#fff', padding: '14px', borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 4 },
  registerText: { textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,0.5)', marginTop: 24 },
};
